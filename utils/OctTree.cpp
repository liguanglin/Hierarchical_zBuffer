...
bool OctTree::check_bound(face3d face, boundingbox3f bb)  //check if in overlap with one of the eight sons
{
...
void OctTree::update(int face_id)   //update face into the octtree
{
    update(face_id, bb, root);
}

void OctTree::update(int face_id, boundingbox3f bb, Node* &node)   //update
{
    if(node == NULL){
        cnt++;
        node = new Node(bb);
    ...
    node->faceNum++;
    float axis[3][3]={            //init bounding box
    ...
    for(int i=0; i<2; i++)
    for(int j=0; j<2; j++)
    for(int k=0; k<2; k++){      // check the eight sons
...
OctTree::OctTree(ObjData *_obj)  //init the octtree
{
    root = NULL;
    obj = _obj;
