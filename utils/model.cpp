...
void ObjData::setT(float _T[][4])  //set tranformation for the obj points
{
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            T[i][j] = _T[i][j];
...
point3f ObjData::getVertex(int idx){  //get the vertex from the obj file
...
ObjData::ObjData(std::string fileName)  //used to save .obj file
{
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            T[i][j] = (i==j);
